...
const AddService = ({ navigation, route }) => {
  const editData = route.params ? route.params.service : null;
  ...
  const price = useInput(editData ? `${editData.price}` : "");
  ...
  const handleSave = () => {
    ...
    if (isNaN(order.value)) {
      setError("الرجاء إدخال رقم في خانة ترتيب الخدمة");
      setLoading(false);
      return;
    }
    if (isNaN(price.value)) {
      setError("الرجاء إدخال رقم في خانة سعر الخدمة");
      setLoading(false);
      return;
    }
    ...
    const data = editData
      ...
      : {
          ...
          order: parseInt(order.value),
          price: parseInt(price.value),
          dep_id: route.params.dep.id,
        };
    form.append("data", JSON.stringify(data));

    if (editData) {
      axios
        .patch(api.services, form, {
          headers: {
        ...
        .then((res) => {
          Alert.alert("", "تم تعديل الخدمة");
          navigation.navigate("singleDep", { dep: route.params.dep });
        })
        .catch((err) => console.log(err))
        .finally(() => setLoading(false));
    } else {
      axios
        .post(api.services, form, {
          headers: {
            Authorization: "bearer " + token,
            "Content-Type": "multipart/form-data",
          },
        ...
        .then((res) => {
          Alert.alert("", "تم إضافة إضافة الخدمة");
          navigation.goBack();
        })
        .catch((err) => console.log(err))
        .finally(() => setLoading(false));
  ...
  return (
    ...
    >
      ...
      <ScrollView>
          ...
          <Avatar
            ...
            source={{
              uri:
                image.value !== ""
                  ? !image.value?.uri
                    ? api.uploads + "/services/" + editData.image
                    : image.value.uri
                  : null,
            }}
            style={{
        ...
        <Input {...title} {...inputProps()} label="عنوان الخدمة" />
        <Input {...desc} {...inputProps()} label="وصف الخدمة" />
        <Input
          {...order}
          {...inputProps()}
          label="ترتيب الخدمة"
        ...
        >
          ...
          >
            {isActive.value ? "فعالة" : "غير فعالة"}
          </Text>
          <Switch
